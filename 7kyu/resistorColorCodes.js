...
function decodeResistorColors(bands) {
  ...
  const colorBreakdown = bands.split(" ");
  if (colorBreakdown.length > 3) {
    colorBreakdown[3] == "gold" ? (percent = 5) : (percent = 10);
  }
  for (let i = 0; i < colorCode.length; i++) {
    if (colorCode[i] == colorBreakdown[0]) {
      num = i.toString();
    }
    if (colorCode[i] == colorBreakdown[1]) {
      secondDigit = i;
    }
    if (colorCode[i] == colorBreakdown[2]) {
      const zero = "0";
      if (i >= 3 && i < 6) {
        k = true;
        multiplier = zero.repeat(i - 3);
      } else if (i >= 6) {
        m = true;
        multiplier = zero.repeat(i - 6);
      } else {
        multiplier = zero.repeat(i);
      }
    }
  }
  let base = num + secondDigit;
  if (!k && !m) {
    if (
      colorCode[2] == colorBreakdown[2] &&
      colorCode[0] == colorBreakdown[1]
    ) {
      return `${num}k ohms, ${percent}%`;
    } else if (colorCode[2] == colorBreakdown[2]) {
      return `${num}.${secondDigit}k ohms, ${percent}%`;
    }
    return `${+base + multiplier} ohms, ${percent}%`;
  } else if (k) {
    if (
      colorCode[5] == colorBreakdown[2] &&
      colorCode[0] == colorBreakdown[1]
    ) {
      return `${num}M ohms, ${percent}%`;
    }
    return `${+base + multiplier}k ohms, ${percent}%`;
  } else if (m) {
    return `${+base + multiplier}M ohms, ${percent}%`;
  }
}
